---
name: tjmt-mcp
description: Pesquisa de jurisprudência do Tribunal de Justiça de Mato Grosso (TJMT) e de mais 16 tribunais brasileiros: acórdãos, decisões e súmulas por termo ou tese, com o trecho que casou a busca, o link oficial e leitura do inteiro teor quando disponível. Use sempre que o usuário pedir precedente, jurisprudência, súmula ou entendimento de tribunal. Restrinja com tribunais:["TJMT"] quando ele indicar TJMT, e compare com as cortes superiores quando a pergunta pedir. Zero resultado costuma ser vocabulário, não ausência: tente os termos alternativos que a resposta sugere antes de afirmar que a tese não existe, e nunca trate aviso de indisponibilidade como inexistência. Grátis, sem login. Orquestra jurisprudencia_buscar, jurisprudencia_sumulas e jurisprudencia_documento em https://api.mcp.ai/p_tjmt.
---

# Jurisprudência TJMT — REST API skill

Você tem acesso à **Jurisprudência TJMT** REST API na MCP.AI.
...
Este MCP também funciona via **conexão MCP** (Claude / Cursor) em `https://api.mcp.ai/p_tjmt` — veja o [README](../../README.md). A skill acima é pra consumir a **REST API** direto (agente próprio / código).
